...
| `pdf_source` | string | **Full-text provenance.** The acquisition tier that supplied the document the row was coded from: `row_url` (the row's own URL, downloaded directly — no index, no DOI needed), `arxiv`, `osf`, `openalex_oa`, `unpaywall_pdf`, `semanticscholar`, `core`, `europepmc`, `landing_<host>`, `serpapi`, `playwright`, or one of the four tiers that hand back a sections dict instead of a file (`_STRUCTURED_SOURCES` in `shared/pdf_sources.py`): `openalex_xml` (the OpenAlex GROBID XML), `epmc_xml` (Europe PMC's JATS full text, the Europe PMC tier's first route), `osf_registration` (the OSF registration form, from the API) and `html_landing` (the row's own page, parsed with lxml). **Blank** when the ladder resolved before Stage 5 or acquired nothing — a `llm_fulltext` row with a blank `pdf_source` is a contradiction and should be treated as unverified |
| `pdf_url` | string | **Full-text provenance.** The URL the document was actually fetched from, as `acquire_pdf` reported it. This is the column that makes a full-text row checkable by hand: a reader who doubts an outcome can open exactly what the model read, rather than re-deriving it from `doi_r` and guessing which tier won. **Blank** whenever `pdf_source` is — the three full-text columns are blank together or not at all. Not necessarily stable: a signed or session-scoped download URL may expire, so it records where the document came from, not a permanent address |
| `pdf_name` | string | **Full-text provenance.** WHICH of that tier's documents the row was read from — an OSF project routinely holds several files, and `pdf_source = `osf_files`` alone does not say which one. The file's own name where the tier reported one (OSF storage lists them), otherwise the URL's last path segment when it names a file. **Blank** whenever `pdf_source` is, and also for a structured source (there is no file) or a guid-shaped download URL (`osf.io/download/abc123/`) the tier gave no name for |
| `parse_method` | string | **Full-text provenance.** The parser whose result won `best_parse_result()` and therefore produced the text the LLM read: `openalex_xml`, `epmc_xml`, `pdfminer`, `grobid`, `docpluck`, `opendataloader` or `markitdown`, or `docx` for a Word file. A structured source names itself here, so `epmc_xml` rows carry it in both columns. Blank when nothing was parsed. `docx` is a FORMAT, not a source — a preprint server serves a Word file as readily as a PDF, and `pdf_source` still names the acquisition tier |
