...
**pending**. Everything marked done was verified against the code on `main` at
`05b22ea`.
...
| Item | Outcome |
| ---- | ------- |
| `shared/llm_client.py` — unused `from pathlib import Path` | **done** — removed with the Gemini call-loop merge |
...
| `link_original.py` — the journal-hint branch of citation scoring | **done** — `_fetch_journal_cached`, `_JOURNAL_TAIL_RE` and the +3.0/+1.5 weights are deleted. The rule itself stays: `_resolve_rule_based` still returns `citation_context_match` when author+year scores ≥4.0 with a ≥2.0 gap, now on the author/year/Jaccard score alone. Production had observed zero firings even with the journal weights |
| `link_original.py` — `_resolve_by_title_pattern`'s dead base-vs-None return shape | **done** — one return shape, `dict | None` |
| `spec.py` — `MatchBlock.pyre_regex` | **done** — retired. Nothing evaluated it and no shipped rule carried it; a `pyre_regex` key is now an unknown-key error, and a decomposed rule's lookaround original lives in `rule_ideas.md` beside the arms that replaced it |
...
| Item | Outcome |
| ---- | ------- |
| `overlay.py:111-114` sizing comment implying a millions-of-rows worklist | **done** — the comment now gives the real figure: the wide case is ~347k textless works out of a 5.1M pool, not millions |
...
| `llm_client.py` — "Gemini (primary)" / "OpenAI (fallback)" section headers | **done** — gone with the Gemini call-loop merge; there is no primary or fallback |
| `release.py:21` — comment implying `RELEASE_INPUTS` order matters | **done** — the comment now says the order is for a reader only, because `_release_id()` sorts the keys before hashing, and that adding or removing a name IS a release-id change |
...
| `osf-registration-protocol.json` — description is a long changelog | **done** — rewritten as a description, 275 words |
...
| `_clean_study_number` / `_clean_study_numbers` | **done** — merged into one `_clean_study_numbers()` |
...
| `link_original.py` — `_study_count_stated` using `.search` | **done** — every match of every pattern is inspected, so a leading year no longer shadows a real count later in the same text |
