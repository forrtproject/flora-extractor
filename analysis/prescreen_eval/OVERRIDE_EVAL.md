...
> **Status (issue #168).** The sixteen tier-A patterns below now ship in
> `_SIGNAL_PATTERNS`. **Every number in this file was measured on the Stage-3-era
> population** — the pre-screen then sat in front of every row entering Stage 3. It no
> longer does: the tier runs over the `screen_cheap` pile the Stage 2 rule book routes
> to it, a smaller population selected by rules that did not exist when this ran. The
> bypass rate and the marginal screen cost must both be re-measured over that pile
> before these figures are quoted as live, and that re-measurement is blocked on the
> cheap-tier promotion decision (current-state review, finding F7: every `screen_cheap`
> spec is still shadow, so today the patterns move no live row). What the held-out
> split does establish, and what does not depend on the denominator, is that the
> patterns are FLoRA's actual vocabulary rather than overfitted phrasings.
...
`override_positives.json` and `override_negatives.json` (16 MB together) are not tracked
here — `build_override_sets.py` rebuilds them from the curated files and `filtered.csv`,
and the copies this file's tables were computed from are on the kept branch
`origin/issue-130-prescreen-followup`. Rebuilding them today does NOT reproduce the
tables: `filtered.csv` is now the Stage 2 engine's screened-only handoff, a different and
much smaller corpus than the 2,581,092-row file below.
