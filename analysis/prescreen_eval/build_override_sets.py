...
two case sets are much larger and independent of that derivation:
  ...
  override_positives.json — every FLoRA replication/reproduction paper with a title and
      an abstract, from all_replications.csv, flora.csv, FLORA_SHEET_PATH (the FLoRA
      entry sheet, named in shared/config.py) and
...
from shared.config import DATA_DIR as DATA, FLORA_SHEET_PATH  # noqa: E402
from shared.prescreen import PRESCREEN_MIN_ABSTRACT_CHARS  # noqa: E402
...
def collect_positives() -> dict[str, dict]:
    ...
    for r in read_csv(FLORA_SHEET_PATH):
