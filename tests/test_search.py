...
def test_fetch_openalex_candidates_writes_the_candidates_schema(monkeypatch, tmp_path):
    """The batch handed to Stage 1's merge must carry exactly the CSV contract's
    columns; the per-field mapping is pinned by test_extract_row_maps_expected_fields."""
    ...
    monkeypatch.setattr(oa.requests, "get",
                        lambda url, params, timeout, **kw: DummyResponse(make_payload()))
...
class TestCursorCompleteness:
    ...
    def test_empty_result_set_is_marked_complete_and_not_refetched(self, monkeypatch, tmp_path):
        ...
        empty = {"meta": {"count": 0, "next_cursor": None}, "results": []}
        n = {"calls": 0}

        def counting_get(url, params, timeout, **kw):
            n["calls"] += 1
            return DummyResponse(empty)

        rows, state = self._run(monkeypatch, tmp_path, empty)
        assert rows == []
        assert state["completed"] is True
        assert state["api_total"] == 0

        monkeypatch.setattr(oa.requests, "get", counting_get)
        oa.fetch_phrase("direct replication", 2020, 2020)
        assert n["calls"] == 0, "a job already known to be empty must not be re-requested"
