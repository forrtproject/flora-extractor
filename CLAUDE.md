...
| `shared/prescreen.py`       | The cheap pre-screen: `hard_signal()`, `prescreen_bypass()`, `prescreen()`. On unless `PRESCREEN_ENABLED=0` |
...
**The cheap pre-screen** (`shared/prescreen.py`, on unless `PRESCREEN_ENABLED=0`)
sits in front of everything below. Two very small models
...
PRESCREEN_ENABLED=1             # on by default; its discards are terminal
PRESCREEN_MODE=discard          # 'shadow' records verdicts without acting on them
